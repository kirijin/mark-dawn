...
if (Get-Command podman -ErrorAction SilentlyContinue) {
    ...
    Write-Error "podman or docker required. Install Podman Desktop: https://podman-desktop.io/"
    exit 1
}
...
switch ($Command) {
    "start" {
        ...
        & $RUNTIME run -d `
            ...
            $IMAGE watcher
        Write-Host "OK: Watcher started" -ForegroundColor Green
    ...
    "install-task" {
        $bat = @"
@echo off
$RUNTIME run --rm --name mark-dawn-task -v "$DATA_DIR\Inbox:/data/Inbox:Z" -v "$DATA_DIR\Research:/data/Research:Z" -v "$DATA_DIR\Inbox_Failed:/data/Inbox_Failed:Z" $IMAGE watcher
"@
        ...
        Write-Host "OK: Task Scheduler job installed and started" -ForegroundColor Green
    }
    
    "uninstall-task" {
        Unregister-ScheduledTask -TaskName "mark-dawn" -Confirm:$false
        Write-Host "OK: Task Scheduler job uninstalled" -ForegroundColor Green
    }
    ...
    default {
        ...
        Write-Host "  install-task       Install as Task Scheduler job (auto-start)"
        Write-Host "  uninstall-task     Uninstall Task Scheduler job"
        Write-Host ""
        Write-Host "Supported formats: PDF, DOCX, XLSX, PPTX, HTML, CSV, RTF" -ForegroundColor Yellow
        ...
        Write-Host "  Documents\Inbox        - Drop files here for auto-conversion"
        Write-Host "  Documents\Research     - Converted markdown files appear here"
        Write-Host "  Documents\Inbox_Failed - Failed conversions moved here"
        Write-Host ""
        Write-Host "Environment:" -ForegroundColor Yellow
        Write-Host "  MARK_DAWN_IMAGE - Docker image to use (default: $IMAGE)"
    }
}
